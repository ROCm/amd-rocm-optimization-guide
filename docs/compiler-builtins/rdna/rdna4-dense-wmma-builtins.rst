.. meta::
   ...
   :keywords: RDNA4, gfx1200, gfx1201, WMMA, dense matrix, wave-matrix,
      HIP builtins, FP32, FP16, BF16, FP8, BF8, INT8, INT4,
      __builtin_amdgcn_wmma, global_load_tr

.. _rdna4-dense-wmma-builtins:
...
:ref:`sparse WMMA <rdna4-sparse-wmma-builtins>` variants that halve the
:math:`\pmb{A}` bandwidth using 2:4 structured sparsity.

CDNA (Instinct) GPUs provide a comparable dense operation through
:doc:`Matrix Fused Multiply-Accumulate (MFMA) <../cdna/dense-mfma-builtins>`.  The two differ in wavefront
...
Equivalent dense WMMA builtins for the previous generation are documented on
the :ref:`rdna3-dense-wmma-builtins` page.
...
``_w32``
    Wavefront size suffix.  All RDNA4 WMMA builtins use wave32.
...
.. figure:: ../../data/compiler-builtins/rdna/wmma-builtins/wmma-layout-d-16x16-rdna4.svg
...
.. _rdna4-dense-wmma-builtin-reference:

Builtin reference
===================

The following sections list every dense WMMA builtin available on RDNA4,
