.. meta::
   :description: Reference for AMD GPU conversion and packing builtins,
      including cvt_pk, cvt_pkrtz, cvt_pknorm, and cvt_sr for FP8/BF8 with architecture availability.
   :keywords: AMD, ROCm, HIP, conversion, packing, cvt_pk, cvt_sr, FP8, BF8, stochastic rounding, builtins, CDNA, RDNA

.. _conversion-packing-builtins:

********************************************************************************
Conversion and packing builtins
********************************************************************************

The conversion and packing builtins convert between floating-point and
integer types and pack multiple values into a single 32-bit register.  They
bridge float pipelines with the byte-oriented arithmetic builtins (SAD, dot
...
Several parameters appear across multiple conversion builtins:
...
``byte_sel`` (byte position selector)
   ...
   ``cvt_pk_u8_f32`` and the single-element FP8/BF8 conversion builtins
...
``rand`` (stochastic rounding seed)
   A 32-bit random value used for stochastic rounding.  The builtin
...
These builtins convert FP32 values to packed unsigned byte representations.
...
These builtins convert pairs of FP32 values to packed FP16 vectors.
...
These builtins pack pairs of 32-bit integer values into 16-bit vector types.
...
These builtins convert between fixed-point nibble values and FP32.
...
These builtins convert FP32 values to normalized 16-bit integers and pack them into vectors.
...
These builtins convert between FP32 and 8-bit floating-point formats (FP8 E4M3 and BF8 E5M2).
...
These builtins convert FP32 values to reduced-precision formats using stochastic rounding.
