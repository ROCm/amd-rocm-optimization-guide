...
// ── Layout functions ──────────────────────────────────────────────────────
// VGPR index: row within its 8-row lane group
...
#let data-grid = grid(
  ...
  column-gutter: gap,
  row-gutter:    gap,
  ..range(16).map(i => range(16).map(j => swmmac-cell(i, j))).flatten()
)
...
#let caption-text = [
  ...
  Applies to all 16×16 SWMMAC intrinsics on RDNA 4 (gfx1200, gfx1201), wave32. \
