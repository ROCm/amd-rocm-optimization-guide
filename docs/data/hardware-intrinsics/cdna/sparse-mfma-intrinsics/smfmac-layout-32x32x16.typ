// SMFMAC 32×32×16 (16-bit inputs) register layout
//
// Four sub-figures, left to right:
//   1. A (sparse, 2:4): rows = matrix row i (0..31), cols = compressed k (0..15)
//   2. Compression index: rows = i (0..31), cols = compressed k (0..15)
//   3. B (dense):  rows = k (0..15), cols = matrix col j (0..31)
//   4. D (output): rows = i (0..31), cols = j (0..31)
//
// Lane mapping:
//   A / Idx : lane = i + 32·⌊k/8⌋   (two k-blocks of 8)
//   B       : lane = 32·⌊k/8⌋ + j   (two k-blocks of 8, j = 0..31)
...
//   VGPR = ⌊k/4⌋ mod 2;  colour group = ⌊k/8⌋  (2 groups)
// ─────────────────────────────────────────────────────────────────────────────
#let a-cell(i, k) = {
...
// ─────────────────────────────────────────────────────────────────────────────
// B (dense) sub-figure
//   rows = k (0..15), cols = j (0..31)
//   VGPR = ⌊k/2⌋ mod 4;  half = k mod 2 → l or h
//   colour group = ⌊k/8⌋  (2 groups)
// ─────────────────────────────────────────────────────────────────────────────
#let b-cell(k, j) = {
...
#let fig-b = sub-fig(
  "B (dense)",
  col-hdr(32),
  row-labels(range(16).map(k => "k=" + str(k))),
  b-data-grid,
  b-total,
)
...
// ─────────────────────────────────────────────────────────────────────────────
#let caption-text = [
  SMFMAC 32×32, 16-bit input elements — operand register layout. \
  ...
  *B* (dense): 4 VGPRs (v0–v3), lo/hi 16-bit half per k-step;
  VGPR = ⌊k/2⌋ mod 4, half = k mod 2, lane = 32·⌊k/8⌋ + j. \
  *D*: 16 VGPRs (v0–v15); VGPR = 4·⌊i/8⌋ + (i mod 4),
  lane = (32·⌊i/4⌋) mod 64 + j.
  Colour groups of 8 k-columns (A/B) or 8 i-rows (D) alternate teal/grey.
]
