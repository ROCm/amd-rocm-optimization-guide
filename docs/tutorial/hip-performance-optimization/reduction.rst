...
Implementing reductions on GPUs requires understanding GPU parallel execution models. GPUs execute thousands of threads simultaneously, organized into groups called warps (on NVIDIA hardware) or wavefronts (on AMD hardware). Threads within a warp/wavefront execute in lockstep, meaning they all execute the same instruction at the same time on different data. This document explores aspects of low-level optimization leveraging this inherent thread model, and refrains from using cooperative groups.
