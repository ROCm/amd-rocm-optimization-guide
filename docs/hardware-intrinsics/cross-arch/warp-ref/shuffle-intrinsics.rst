.. meta::
   :description: Reference for AMD GPU shuffle and lane access intrinsics,
      including __shfl, readlane, readfirstlane, and writelane with usage and architecture availability.
   :keywords: AMD, ROCm, HIP, shuffle, readlane, readfirstlane, writelane, warp, lane access, intrinsics, CDNA, RDNA

.. _shuffle-intrinsics:

********************************************************************************
Shuffle and lane access intrinsics
********************************************************************************

Shuffle intrinsics copy a value from one lane to another within a warp.  The
``__shfl*`` family provides a higher-level interface, while the
``__builtin_amdgcn_*`` hardware intrinsics map directly to AMD Instruction Set Architecture (ISA)
...
These intrinsics provide a higher-level interface for copying values between lanes within a warp.
...
These intrinsics map directly to AMD ISA instructions for reading and writing individual lane values.
...
intrinsic useful for promoting a per-lane value to a uniform value for use as
