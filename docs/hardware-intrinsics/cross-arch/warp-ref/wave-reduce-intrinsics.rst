.. meta::
   :description: Reference for AMD GPU single-instruction warp reduction intrinsics, including wave_reduce_add, wave_reduce_min, wave_reduce_max, and architecture availability.
   :keywords: AMD, ROCm, HIP, wave reduce, warp reduction, wave_reduce_add, wave_reduce_min, wave_reduce_max, intrinsics, CDNA, RDNA

.. _wave-reduce-intrinsics:

********************************************************************************
Warp reduction intrinsics
********************************************************************************

The warp reduction intrinsics compute a single scalar result from per-lane
values across all active lanes in a single intrinsic call.  They encapsulate
...
All warp reduction intrinsics share the same two-parameter interface:

``src`` (per-lane input)
   The value contributed by each lane.  The type (``unsigned int``,
   ``unsigned long``, ``int``, or ``long``) matches the intrinsic's type
   suffix.
...
These intrinsics compute the sum of per-lane values across all active lanes.
...
These intrinsics compute a subtraction reduction across all active lanes.
...
These intrinsics find the minimum value across all active lanes.
...
These intrinsics find the maximum value across all active lanes.
...
These intrinsics compute the bitwise AND of all active lanes.
...
These intrinsics compute the bitwise OR of all active lanes.
...
These intrinsics compute the bitwise XOR of all active lanes.
