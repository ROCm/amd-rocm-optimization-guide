...
.. list-table::
   ...
   * - ``blgp``
     ...
     - :math:`\pmb{B}`-matrix Lane Group Pattern modifier. Allows a
       ...
       **FP64 exception:** for FP64 MFMA intrinsics on CDNA3, this parameter is
       repurposed as a 3-bit NEG modifier. Each bit negates the corresponding
       source operand (bit 0: negate :math:`\pmb{A}`; bit 1: negate
       :math:`\pmb{B}`; bit 2: negate :math:`\pmb{C}`).  Lane-group pattern
       swizzling is not available for FP64 on CDNA3.
